...
class WordPressPoster:	
	def __init__(self, date, primary_category_name, secondary_category_name=None):		
		self.client = Client('http://www.instagoals.com/xmlrpc.php', 'admin', 'somepassword')
		self.date = date
		self.primary_category_name = primary_category_name
		if secondary_category_name != None:
			self.secondary_category_name = secondary_category_name + " - " + self.format_date(date)
		else:
			self.secondary_category_name = None
...
def main():
	p = argparse.ArgumentParser(description='Create a HTML page containing a GIF.')
